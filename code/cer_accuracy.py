...
transcription = "信息技术部门中机器学习的主要应用之一是向潜在用户或客户推荐项目就可以分为两种主要应用在线广告和项目建议通常这些建议的目的仍然是为了销售产品两者都依赖于预测用户和项目质量关联一旦向该用户展示了广告后推荐该产品推荐系统要么与这些行为的概率"
ground_truth_punc = remove_punctuation(ground_truth)
print(f"transcription without punctuation: {ground_truth_punc}")

# 计算准确率
acc = accuracy(ground_truth, transcription)
acc_punc = accuracy(ground_truth_punc, transcription)
print(f"Accuracy: {acc:.2f}")
print(f"Accuracy without punctuation: {acc_punc:.2f}")

# 计算CER
CER = cer(ground_truth, transcription)
CER_punc = cer(ground_truth_punc, transcription)
print(f"CER: {CER:.2f}")
print(f"CER without punctuation: {CER_punc:.2f}")
